...
pub struct KeyValuePair<T: Sized> {
    key: String,
    value: T,
}
...
pub struct EmbeddedDatabase<T: Sized> {
    data: IndexMap<String, KeyValuePair<T>, MAX_SIZE>,
}
...
impl<T: Sized> Database<T> for EmbeddedDatabase<T> {
    fn insert(&mut self, key: String, value: T) -> Result<(), String> {
        self.data.insert(key.clone(), KeyValuePair { key, value })
            .map(|_| ())
    ...
    fn remove(&mut self, key: &str) -> Option<T> {
        self.data.remove(key).map(|kv| kv.value)
    }
}
#[cfg(test)]
mod tests {
